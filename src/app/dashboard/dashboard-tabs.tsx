...
function statusColor(status: string): string {
  ...
  if (status === "escalated") return "text-amber-700 bg-amber-50 border-amber-200";
  if (status === "approved") return "text-green-700 bg-green-50 border-green-200";
  if (status === "overridden") return "text-purple-700 bg-purple-50 border-purple-200";
...
function CampaignView({ campaigns }: { campaigns: Campaign[] }) {
  ...
  return (
    <div className="space-y-6">
      {campaigns.map((c) => (
        <div key={c.id} className="bg-white border border-neutral-200 rounded-lg overflow-hidden">
          <div className="px-6 py-5 border-b border-neutral-200 bg-neutral-50">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-lg font-medium text-neutral-900">{c.name}</h3>
                {c.description && (
              ...
              <div className="flex gap-3 text-xs text-neutral-600">
                <span>
                  <span className="font-medium text-neutral-900">{c.totalDrafts}</span> draft{c.totalDrafts === 1 ? "" : "s"}
                </span>
                {c.blocked > 0 && (
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {c.speakers.map((sp) => (
                <div key={sp.id} className="border border-neutral-200 rounded-lg p-4">
                  <div className="mb-3">
                    <div className="text-sm font-medium text-neutral-900">{sp.name}</div>
                    <div className="text-xs text-neutral-500">{sp.title || ""}</div>
                  </div>
                      ...
                      <li key={d.id}>
                        ...
                        >
                          <div className="flex items-start gap-2">
                            ...
                            <p className="text-xs text-neutral-700 group-hover:text-neutral-900 line-clamp-2">
                              {d.draft_text}
                            </p>
                          </div>
                        </Link>
      ...
      <table className="w-full text-sm">
        <thead className="bg-neutral-50 border-b border-neutral-200">
          <tr>
            ...
            <th className="text-right px-4 py-3 font-medium text-red-700">Blocked</th>
            <th className="text-right px-4 py-3 font-medium text-amber-700">Escalated</th>
            <th className="text-right px-4 py-3 font-medium text-green-700">Approved</th>
            <th className="text-right px-4 py-3 font-medium text-purple-700">Overridden</th>
          </tr>
        </thead>
        <tbody>
          {speakers.map((s) => (
            <tr key={s.id} className="border-b border-neutral-100 last:border-0 hover:bg-neutral-50">
              ...
              <td className="px-4 py-3 text-right text-neutral-900 font-medium">{s.totalDrafts}</td>
              <td className="px-4 py-3 text-right text-red-700">{s.blocked || ""}</td>
              <td className="px-4 py-3 text-right text-amber-700">{s.escalated || ""}</td>
              <td className="px-4 py-3 text-right text-green-700">{s.approved || ""}</td>
              <td className="px-4 py-3 text-right text-purple-700">{s.overridden || ""}</td>
            </tr>
          ))}
        </tbody>
      </table>
